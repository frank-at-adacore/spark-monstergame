...
package body Game_Controller is
   ...
   begin
      ...
      for I in 1 .. Number_Of_Obstacles_To_Add loop
         ...
         Cell_To_Place_Piece :=
           Game_Board.Get_Random_Cell (Find_Empty_Cell => True);
   ...
   procedure Get_Player_Move
     (Move : out Game_Board.Movement_Direction;
      Quit : out Boolean) is
   ...
   begin
      if Move_Counter >= Number_Of_Moves_To_Display then
         Screen.Move_Cursor
           (X => 0,
            Y =>
              Game_Board.Num_Rows * Game_Board.Row_Height
      ...
      while not Valid_Move loop
         Text_IO.Put
           ("Enter Player Movement Direction (N,NE,E,SE,S,SW,W,NW) or Q to quit: ");
         Text_IO.Get_Line
           (Item => Line,
            Last => Last);
         declare
