...
package body Game_Board is
   ...
   package Random_Direction is new Random_Generic
     (Result_Subtype => Movement_Direction);

   -- Game board is 2-D array of cells.
   type Cell_Array is
     array (Row_Range, Col_Range) of aliased Game_Board_Cell.Cell_Class_Ptr;
   ...
   begin
      ...
      for I in Row_Range loop
         ...
         for J in Col_Range loop
            Board_Cells (I, J) := Game_Board_Cell.Make
                (Its_Location =>
                   (Row => I,
                    Col => J),
                 Height       => Row_Height,
   ...
   begin
      ...
      for I in 1 .. Row_Height loop
         Screen.Move_Cursor
           (X => Board_Display.X (Offset => Cell.Board_Location.Col),
   ...
   procedure Move_Piece
     (From_Rom  : in     Row_Range;
      ...
      Status    :    out Move_Request_Status_Type) is
   ...
   begin
      ...
      if Piece = null then
         ...
         Status := Invalid;
         Text_IO.Put_Line
           ("Cannot move a player piece that is not on the board!");
      else
         -- Attempt the move.
         Move_Piece
           (From_Rom  => Player_R,
   ...
   procedure Move_Monsters (Monster_Ate_Player : out Boolean) is
         ...
         for C in Col_Range loop
            ...
            if Get_Piece_Result then
               ...
               if Get_Piece_Result then
                  ...
                  declare
                     Monster : Pieces.Pawn.Monster.Monster_Class_Ptr :=
                       Pieces.Pawn.Monster.Monster_Class_Ptr (Monster_Piece);
